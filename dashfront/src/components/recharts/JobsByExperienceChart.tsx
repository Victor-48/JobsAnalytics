...
export const JobsByExperienceChart = ({ displayType = 'bar', displayUnit = 'absolute' }: { displayType: 'pie' | 'bar', displayUnit: 'percentage' | 'absolute' }) => {
    const { data, isLoading } = useAnalytics();
    const total = calculateTotal(data.jobsByExperience, 'count');
    ...
    return displayType === 'pie' ? (
        ...
        <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.jobsByExperience} margin={{ top: 20, right: 10, left: 10, bottom: 20 }}>
                ...
                <YAxis tickFormatter={(v) => formatValue(v, total, displayUnit).toString()} tick={{fontSize: 11, fill: 'hsl(var(--muted-foreground))'}} axisLine={false} tickLine={false} dx={-10} />
                <Tooltip
                    formatter={(value: any) => [formatValue(value, total, displayUnit), 'Jobs']}
                    contentStyle={{ borderRadius: '8px', border: '1px solid hsl(var(--border))', backgroundColor: 'hsl(var(--card))', color: 'hsl(var(--card-foreground))' }}
                />
